Partial Public Class dbLOTES_COSECHA
    ...
    Public Overrides Function Agregar(ByVal aEntidad As entidadBase) As Integer
        ...
        'Return SqlHelper.ExecuteNonQuery(Me.cnnStr, CommandType.Text, strSQL.ToString(), args)
        Return ACTUALIZAR_LOTES_COSECHA_APP(DirectCast(aEntidad, LOTES_COSECHA))
    End Function

    Private Function ACTUALIZAR_LOTES_COSECHA_APP(ByVal aEntidad As LOTES_COSECHA) As Int32
        Dim args As New List(Of SqlParameter)
        Dim arg As New SqlParameter

        arg = New SqlParameter("@ID_LOTES_COSECHA", SqlDbType.Int)
        arg.Value = aEntidad.ID_LOTES_COSECHA
        args.Add(arg)
        ...
        args.Add(arg)
        Return SqlHelper.ExecuteNonQuery(Me.cnnStr, CommandType.StoredProcedure, "ACTUALIZAR_LOTES_COSECHA_APP", args.ToArray)
    End Function

    Public Function ActualizarReferenciasLOTES_COSECHA(ByVal ACCESLOTE_OLD As String, ByVal ACCESLOTE_NEW As String) As Int32
        Dim lRet As Int32 = 0
        ...
        Try
            SqlHelper.ExecuteNonQuery(Me.cnnStr, "ACTUALIZAR_REFERENCIAS_LOTES_COSECHA", args)
            Return 1
    ...
    Public Function ObtenerUltimaCosechaPorLOTE_FinLote(ByVal ID_MAESTRO As Int32, ByVal ESTADO_LOTE As String, Optional ID_ZAFRA As Int32 = -1) As listaLOTES_COSECHA
        ...
        strSQL.Append("FROM LOTES_COSECHA LC, ZAFRA Z ")
    ...
    Public Function ObtenerLOTES_COSECHAPorLOTE_ZAFRA(ByVal ACCESLOTE As String, ByVal ID_ZAFRA As Int32) As LOTES_COSECHA
        ...
        strSQL.Append("FROM LOTES_COSECHA ")
    ...
    Public Function ObtenerLOTES_COSECHAPorMAESTRO_ZAFRA(ByVal ID_MAESTRO As Int32, ByVal ID_ZAFRA As Int32) As LOTES_COSECHA
        ...
        strSQL.Append("FROM LOTES_COSECHA LC, LOTES L ")
    ...
    Public Function ObtenerREND_COSECHA_UltimaZafra(ByVal ID_MAESTRO As Int32, ByVal ID_ZAFRA As Int32) As Decimal
        ...
        strSQL.Append("FROM LOTES L, LOTES_COSECHA LC, ZAFRA Z ")
    ...
    Public Function ObtenerListaPorCriterios(ByVal ID_ZAFRA As Int32,
        ...
        arg.Value = If(ZONA = Nothing, "", ZONA)
        args.Add(arg)

        arg = New SqlParameter("@CODIPROVEE", SqlDbType.VarChar)
        arg.Value = If(CODIPROVEE = Nothing, "", CODIPROVEE)
        args.Add(arg)

        arg = New SqlParameter("@CODISOCIO", SqlDbType.VarChar)
        arg.Value = If(CODISOCIO = Nothing, "", CODISOCIO)
        args.Add(arg)

        arg = New SqlParameter("@NOMBRE_PROVEEDOR", SqlDbType.VarChar)
        arg.Value = If(NOMBRE_PROVEEDOR = Nothing, "", NOMBRE_PROVEEDOR.ToUpper)
        args.Add(arg)

        arg = New SqlParameter("@NOMBLOTE", SqlDbType.VarChar)
        arg.Value = If(NOMBLOTE = Nothing, "", NOMBLOTE.ToUpper)
        args.Add(arg)
        ...
        arg = New SqlParameter("@CODIAGRON", SqlDbType.VarChar)
        arg.Value = If(CODIAGRON = Nothing, "", CODIAGRON)
        args.Add(arg)

        arg = New SqlParameter("@USUARIO", SqlDbType.VarChar)
        arg.Value = If(USUARIO = Nothing, "", USUARIO)
        args.Add(arg)

        Dim lista As New listaLOTES_COSECHA

        dr = SqlHelper.ExecuteReader(Me.cnnStr, CommandType.StoredProcedure, "OBTENER_LISTA_LOTES_COSECHA", args.ToArray)

        Try

            Do While dr.Read()
                Dim mEntidad As New LOTES_COSECHA
                dbAsignarEntidades.AsignarLOTES_COSECHA_IncluyeContrato(dr, mEntidad)
                lista.Add(mEntidad)
            Loop
    ...
    '                                         ByVal CODIAGRON As String) As listaLOTES_COSECHA
    '    Dim args As New List(Of SqlParameter)
    '    Dim arg As SqlParameter
    '    Dim strSQL As New StringBuilder
    '    Dim strCond As New StringBuilder

    '    strSQL.Append("SELECT * FROM LOTES_COSECHA LC ")
    ...
    Public Function execXLS_COMPARATIVO_ZAFRA_ACTUAL_VS_ANTERIOR(ByVal idZafraInicial As Integer, ByVal idZafraFinal As Integer) As DataSet
        ...
        arg = New SqlParameter("@ID_ZAFRA1", idZafraInicial)
        args.Add(arg)
        arg = New SqlParameter("@ID_ZAFRA2", idZafraFinal)
        args.Add(arg)

        ds = SqlHelper.ExecuteDataset(Me.cnnStr, CommandType.StoredProcedure, "XLS_COMPARATIVO_ZAFRA_ACTUAL_VS_ANTERIOR", args.ToArray)
