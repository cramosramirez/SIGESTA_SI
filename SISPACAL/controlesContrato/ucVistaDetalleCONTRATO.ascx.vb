...
''' </history>
''' -----------------------------------------------------------------------------
Partial Class controles_ucVistaDetalleCONTRATO
    Inherits ucBase
...
#Region "Propiedades"
    ...
    Public Property LISTA_LOTES As listaLOTES
        Set(value As listaLOTES)
            Session(Me.lblREFERENCIA.Text) = value
        End Set
        Get
            If Session(Me.lblREFERENCIA.Text) IsNot Nothing Then Return TryCast(Session(Me.lblREFERENCIA.Text), listaLOTES) Else Return New listaLOTES
        End Get
    End Property
    ...
    Private mComponente As New cCONTRATO
    Private mEntidad As CONTRATO
...
#End Region
    ...
    Private Sub CargarDatos()
        ...
        Dim lContratoZafras As listaCONTRATO_ZAFRAS
        Dim listaLotesContra As New listaLOTES
        Dim listaLotesProvee As listaLOTES
        Me._nuevo = False
        If Not Me.ViewState("nuevo") Is Nothing Then Me.ViewState.Remove("nuevo")
        Me.ViewState.Add("nuevo", Me._nuevo)

        Dim sError As New String("")
        mEntidad = New CONTRATO
        mEntidad.CODICONTRATO = CODICONTRATO

        If mComponente.ObtenerCONTRATO(mEntidad) <> 1 Then
        ...
        Me.CargarZafra()
        lContratoZafras = (New cCONTRATO_ZAFRAS).ObtenerListaPorCONTRATO(mEntidad.CODICONTRATO)
        If Me.cbxZAFRA.FindControl("listBox") IsNot Nothing AndAlso lContratoZafras IsNot Nothing AndAlso lContratoZafras.Count > 0 Then
            ...
            For Each lContraZ As CONTRATO_ZAFRAS In lContratoZafras
        ...
        If lZafraActiva IsNot Nothing AndAlso lZafraActiva.ID_ZAFRA <> Me.ID_ZAFRA Then
            listaLotesProvee = (New cLOTES).ObtenerListaPorCONTRATO(mEntidad.CODICONTRATO)
        Else
            listaLotesProvee = (New cLOTES).ObtenerListaPorCONTRATO(mEntidad.CODICONTRATO)
        ...
        Me.LISTA_LOTES = listaLotesProvee
        Me.CargarDetalleContrato()
    ...
    Public Sub CargarDetalleContrato()
        If Me.lblREFERENCIA.Text <> "" Then
            Me.ucListaLOTES1.CargarDatosCache(Me.lblREFERENCIA.Text)
        End If
        SetearAREA_TCXAREA()
    End Sub
    ...
    Public Sub SetearAREA_TCXAREA()
        ...
        For Each lLotes As LOTES In LISTA_LOTES
            MZ += lLotes.AREA
            TC += lLotes.TONELADAS
    ...
    Public Sub RefrescarDetalleLotes()
        ...
        Dim lLotesProveedor As New listaLOTES
        Dim lZafra As ZAFRA = (New cZAFRA).ObtenerZafraActiva
        Dim indice As Int32

        If lProveedor IsNot Nothing Then
            lLotesProveedor = Me.LISTA_LOTES
            ...
            For Each lMaestro As MAESTRO_LOTES In lMaestroLotes
                ...
                End If
            Next
            Me.LISTA_LOTES = lLotesProveedor
            Me.CargarDetalleContrato()
        End If
    End Sub
    ...
    Private Sub HabilitarEdicion(ByVal edicion As Boolean)
        ...
        If item IsNot Nothing Then item.Visible = Me._nuevo
        Me.ucListaLOTES1.VerCONTRATO_TRASPASO = (Me._CODICONTRATO <> "")
    End Sub
    ...
    Private Sub Nuevo()
        ...
        Me.LISTA_LOTES = New listaLOTES
        Me.CargarDetalleContrato()
        Me.cbxTIPO_CONTRATO.Value = CInt(Enumeradores.TipoContrato.PersonaNatural)
        Me.ucListaLOTES1.PermitirFilaDeFiltro = False
        Me.ucListaLOTES1.VerCODISOCIO_NUMERICO = False
        Me.ucListaLOTES1.VerNOMBRE_SOCIO = False
        Me.ucListaLOTES1.VerRIESGO_PIEDRA = True
        Me.ucListaLOTES1.VerREPARA_ACCESO = True
        Me.ucListaLOTES1.VerSIN_ACCESO_PROPIO = True
    ...
    Public Function Actualizar() As String
        ...
        Dim bContratoZafras As New cCONTRATO_ZAFRAS
        Dim lotesSelecc As New listaLOTES
        ...
        If Me._nuevo Then
            ...
            End If
        Else
            mEntidad = (New cCONTRATO).ObtenerCONTRATO(Me.txtCODICONTRATO.Text)
            mEntidad.USER_ACT = 5
            mEntidad.FECHA_ACT = cFechaHora.ObtenerFechaHora
        ...
        For Each lLote As LOTES In LISTA_LOTES
        ...
        For Each lLoteVal As LOTES In lotesSelecc
        ...
        For i As Integer = 0 To aZafras.Count - 1
            ...
            If lZafra IsNot Nothing Then lZafras.Add(lZafra)
        Next
        For Each lLote As LOTES In lotesSelecc
            If lLote.ID_LOTE_TRASPASO <= 0 Then
                Dim lLotes As listaLOTES
                For i As Integer = 0 To lZafras.Count - 1
                    If Me._nuevo Then
                        lLotes = (New cLOTES).ObtenerListaPorZAFRA_MAESTRO_LOTES(lZafras(i).ID_ZAFRA, lLote.ID_MAESTRO)
                    Else
                        lLotes = (New cLOTES).ObtenerListaPorZAFRA_MAESTRO_LOTES(lZafras(i).ID_ZAFRA, lLote.ID_MAESTRO, Me.txtCODICONTRATO.Text)
                    End If

                    If lLotes IsNot Nothing AndAlso lLotes.Count > 0 Then
                        Dim listaZafrasContra As listaCONTRATO_ZAFRAS = bContratoZafras.ObtenerListaPorCONTRATO(lLotes(0).CODICONTRATO)
                        ...
                        If listaZafrasContra IsNot Nothing AndAlso listaZafrasContra.Count > 0 Then
                            ...
                            Next
                        End If
                        Dim lContrato As CONTRATO = (New cCONTRATO).ObtenerCONTRATO(lLotes(0).CODICONTRATO)
        ...
        If PERMITIR_MODIFICACION_DATOS Then
            ...
            Dim lLotesContratados As listaLOTES
            lLotesContratados = (New cLOTES).ObtenerListaPorCONTRATO(mEntidad.CODICONTRATO)
            If lLotesContratados IsNot Nothing AndAlso lLotesContratados.Count > 0 Then
                For Each lLoteContra As LOTES In lLotesContratados
                    If lotesSelecc.BuscarPorColumna("ID_MAESTRO", lLoteContra.ID_MAESTRO, True) Is Nothing Then
                        ...
                        If lZafraAnt IsNot Nothing Then
                            For Each eEntidad As ZAFRA In lZafras
            ...
            If mComponente.ActualizarCONTRATO(mEntidad) <> 1 Then
        ...
        If lContratoZafras IsNot Nothing AndAlso lContratoZafras.Count > 0 Then
            ...
            For Each lZafra As ZAFRA In lZafras
                If lContratoZafras.BuscarPorColumna("ID_ZAFRA", lZafra.ID_ZAFRA, True) Is Nothing Then
                    ...
                    bContratoZafras.ActualizarCONTRATO_ZAFRAS(lContratoZafra)
                End If
            Next
            'Eliminar zafras que no existan en las seleccionadas (solo para zafras posteriores a la actual)
            For Each lContraZafra As CONTRATO_ZAFRAS In lContratoZafras
                If lZafras.BuscarPorId(lContraZafra.ID_ZAFRA) Is Nothing Then
                    If CInt((New cZAFRA).ObtenerZAFRA(lContraZafra.ID_ZAFRA).NOMBRE_ZAFRA.Substring(0, 4)) >= CInt(lZafraActiva.NOMBRE_ZAFRA.Substring(0, 4)) Then
                        bContratoZafras.EliminarCONTRATO_ZAFRAS(lContraZafra.ID_CONTRATO_ZAFRAS)
            ...
            If lLotesContratados IsNot Nothing AndAlso lLotesContratados.Count > 0 Then
                ...
                For Each lLote As LOTES In lotesSelecc
                    ...
                    Else
                        ...
                        lLoteContra.FECHA_ACT = cFechaHora.ObtenerFechaHora
                    End If
                    If bLotes.ActualizarLOTES(lLoteContra, TipoConcurrencia.Pesimistica) <= -1 Then
                ...
                For Each lLoteContra As LOTES In lLotesContratados
                    If lotesSelecc.BuscarPorColumna("ID_MAESTRO", lLoteContra.ID_MAESTRO, True) Is Nothing Then
                        bLotes.EliminarLOTES(lLoteContra.ACCESLOTE)
                    End If
                Next
            Else
                For Each lLote As LOTES In lotesSelecc
                    ...
                    lLoteContra.USER_ACT = 1
                    lLoteContra.FECHA_ACT = cFechaHora.ObtenerFechaHora
                    If bLotes.ActualizarLOTES(lLoteContra, TipoConcurrencia.Pesimistica) <= 0 Then
                        Return bLotes.sError
                    End If
                Next
            End If
        Else
            Dim lLotesContratados As listaLOTES
            lLotesContratados = (New cLOTES).ObtenerListaPorCONTRATO(mEntidad.CODICONTRATO)
            For Each lLote As LOTES In lLotesContratados
                If bLotes.ActualizarLOTES(lLote, TipoConcurrencia.Pesimistica) < 1 Then
                    Return bLotes.sError
                End If
            Next
        End If
    ...
    Protected Sub txtCODIPROVEE_V_ValueChanged(sender As Object, e As System.EventArgs) Handles txtCODIPROVEE_V.ValueChanged
        ...
        If txtCODIPROVEE_V.Text <> "" Then
            ...
            If lProveedor IsNot Nothing Then
                ...
                End If
                Me.LISTA_LOTES = Me.ObtenerLotesPorProveedor(sCodiProvee, lZafra.ID_ZAFRA)
            Else
    ...
    Private Function ObtenerLotesPorProveedor(ByVal CODIPROVEEDOR As String, ByVal ID_ZAFRA_ACTUAL As Int32) As listaLOTES
        Dim lProveedor As PROVEEDOR = (New cPROVEEDOR).ObtenerPROVEEDOR(CODIPROVEEDOR)
        Dim bMaestroLote As New cMAESTRO_LOTES
        Dim lMaestroLotes As listaMAESTRO_LOTES
        Dim lLotesProveedor As New listaLOTES
        ...
        If lProveedor IsNot Nothing Then
            ...
            For Each lMaestro As MAESTRO_LOTES In lMaestroLotes
                ...
                If lstLoteCosecha IsNot Nothing AndAlso lstLoteCosecha.Count > 0 Then
                    ...
                    lLoteContratado = (New cLOTES).ObtenerLOTES(lstLoteCosecha(0).ACCESLOTE)
                End If
    ...
    Private Sub ConfigurarVistaPorTipoContrato()
        Me.ucListaLOTES1.PermitirFilaDeFiltro = False
        ucListaLOTES1.VerCODISOCIO_NUMERICO = False
        ucListaLOTES1.VerNOMBRE_SOCIO = False
        Select Case cbxTIPO_CONTRATO.Value
            ...
            Case Enumeradores.TipoContrato.Cooperativa
                Me.ucListaLOTES1.PermitirFilaDeFiltro = True
                ucListaLOTES1.VerCODISOCIO_NUMERICO = True
                ucListaLOTES1.VerNOMBRE_SOCIO = True
                MostrarFormatoContratoJuridico(True)
        End Select
    End Sub
    ...
    Protected Sub btnBuscar_Click(sender As Object, e As System.EventArgs) Handles btnBuscar.Click
        ...
        If cbxTIPO_DERECHO.Text = "" Then
            ...
            Return
        End If
        Dim lContratos As listaCONTRATO = (New cCONTRATO).ObtenerListaPorCriterios(CInt(cbxZafraTraspaso.Value), CInt(Me.txtNoContratoTraspaso.Value), "", "")
        If lContratos IsNot Nothing AndAlso lContratos.Count > 0 Then
            ...
            End If
            Me.lblpcError.Text = ""
            Me.ucListaLOTES2.QuitarSeleccion()
            Me.MostrarLotesTraspasar()
        Else
    ...
    Private Sub MostrarLotesTraspasar()
        If cbxZafraTraspaso.Value IsNot Nothing AndAlso Me.txtNoContratoTraspaso.Text <> "" Then
            Dim lContratos As listaCONTRATO = (New cCONTRATO).ObtenerListaPorCriterios(CInt(cbxZafraTraspaso.Value), CInt(Me.txtNoContratoTraspaso.Value), "", "")
            If lContratos IsNot Nothing AndAlso lContratos.Count > 0 Then
                Me.ucListaLOTES2.CargarDatosPorCONTRATO(lContratos(0).CODICONTRATO)
            End If
        End If
    End Sub
    ...
    Public Function ElegirLoteContratado() As String
        ...
        Me.ucListaLOTES2.QuitarFiltros()
        Me.ucListaLOTES2.CargarDatosPorCONTRATO("")
    ...
    Protected Sub btnSeleccionar_Click(sender As Object, e As System.EventArgs) Handles btnSeleccionar.Click
        Dim lLotes As listaLOTES = Me.ucListaLOTES2.DevolverSeleccionados
        If lLotes Is Nothing OrElse lLotes.Count = 0 Then
            Me.lblpcError.Text = "* Seleccione un lote de la lista."
            Return
        End If
        Dim lLote As LOTES = (New cLOTES).ObtenerLOTES(lLotes(0).ACCESLOTE)
        If lLote IsNot Nothing Then
            lLotes = Me.LISTA_LOTES
            If lLotes Is Nothing Then lLotes = New listaLOTES

            'Verificar que el lote no se encuentre agregado al contrato
            For Each l As LOTES In lLotes
