...
''' -----------------------------------------------------------------------------
''' Project	 : SISPACAL
''' Class	 : ucListaRECIBO_CANA_ANULADO
''' 
''' -----------------------------------------------------------------------------
''' <summary>
''' Clase del Control de Usuario para mostrar un listado de la tabla RECIBO_CANA_ANULADO
''' </summary>
''' <remarks>
...
''' </history>
''' -----------------------------------------------------------------------------
Partial Class controles_ucListaRECIBO_CANA_ANULADO
    Inherits ucBase
 
    Private mComponente As New cRECIBO_CANA_ANULADO
    Public Event Seleccionado(ByVal ID_RECIBO_CANA_ANUL As Int32) 
    Public Event Editando(ByVal ID_RECIBO_CANA_ANUL As Int32) 
...
#Region"Propiedades"
    ...
    Public Property HeaderID_RECIBO_CANA_ANUL() As String
        Get
...
#End Region
    ...
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ''' FunciÃ³n que Carga la informaciÃ³n de los registros de la tabla RECIBO_CANA_ANULADO
    ''' </summary>
    ''' <remarks>
    ''' </remarks>
    ''' <history>
    ...
    Public Function CargarDatos() As Integer
        Dim lRECIBO_CANA_ANULADO As ListaRECIBO_CANA_ANULADO
        lRECIBO_CANA_ANULADO = Me.mComponente.ObtenerLista()
        If lRECIBO_CANA_ANULADO is Nothing Then
            Me.gvLista.Visible = False
            Return -1
        End If
        Me.gvLista.SelectedIndex = -1
        Me.gvLista.Visible = True
        Me.gvLista.DataSource = lRECIBO_CANA_ANULADO
        Me.gvLista.DataBind()
        Return 1
    End Function
    ...
    Public Function CargarDatosPorZAFRA(ByVal ID_ZAFRA As Int32) As Integer
        Dim lRECIBO_CANA_ANULADO As ListaRECIBO_CANA_ANULADO
        lRECIBO_CANA_ANULADO = Me.mComponente.ObtenerListaPorZAFRA(ID_ZAFRA)
        If lRECIBO_CANA_ANULADO is Nothing Then
            Me.gvLista.Visible = False
            Return -1
        End If
    ...
    Public Function CargarDatosPorENVIO(ByVal ID_ENVIO As Int32) As Integer
        Dim lRECIBO_CANA_ANULADO As ListaRECIBO_CANA_ANULADO
        lRECIBO_CANA_ANULADO = Me.mComponente.ObtenerListaPorENVIO(ID_ENVIO)
        If lRECIBO_CANA_ANULADO is Nothing Then
            Me.gvLista.Visible = False
            Return -1
        End If
    ...
    Protected Sub gvLista_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvLista.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim mLabelID_RECIBO_CANA_ANUL As Label
            mLabelID_RECIBO_CANA_ANUL = e.Row.FindControl("Label_ID_RECIBO_CANA_ANUL")
            mLabelID_RECIBO_CANA_ANUL.Visible = Not Me.PermitirEditar
            ...
            If Me.VerID_ENVIO Then
                Dim lblNROBOLETA As Label
                lblNROBOLETA = CType(e.Row.FindControl("lblNROBOLETA"), Label)
                If lblNROBOLETA IsNot Nothing Then
                    Dim lReciboAnulado As RECIBO_CANA_ANULADO = CType(e.Row.DataItem, RECIBO_CANA_ANULADO)
                    If lReciboAnulado IsNot Nothing Then
                        If lReciboAnulado.ID_ENVIO <> -1 Then
                            Dim lEnvio As ENVIO = (New cENVIO).ObtenerENVIO(lReciboAnulado.ID_ENVIO)
                            If lEnvio IsNot Nothing Then
                                lblNROBOLETA.Text = lEnvio.NROBOLETA.ToString
                            End If
                        End If
                    End If
                End If
    ...
    Private Sub gvLista_RowDeleting(ByVal source As System.Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles gvLista.RowDeleting
        If Me.mComponente.EliminarRECIBO_CANA_ANULADO(CInt(CType(Me.gvLista.Rows(e.RowIndex).FindControl("LinkButton1"), LinkButton).ToolTip)) <> 1 Then
    ...
    Public Function DevolverSeleccionados() As ListaRECIBO_CANA_ANULADO
        Dim mLista As New ListaRECIBO_CANA_ANULADO
        For Each mRow As GridViewRow In Me.gvLista.Rows
            If CType(mRow.FindControl("CheckBox_Seleccionar"), CheckBox).Checked Then
                Dim mEntidad As New RECIBO_CANA_ANULADO
                mEntidad.ID_RECIBO_CANA_ANUL = CInt(CType(mRow.FindControl("Label_ID_RECIBO_CANA_ANUL"), Label).Text)
                mLista.Add(mEntidad)
            End If
        Next
        Return mLista
