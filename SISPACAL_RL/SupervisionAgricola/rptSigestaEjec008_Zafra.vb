...
Public Class rptSigestaEjec008_Zafra
    ...
    Public Sub CargarDatos(ByVal TIPO As String, _
        ...
        Me.RPT_SIGESTA_EJEC008_PLAN_COSECHATableAdapter1.FillPorCriterios(DS_SIGESTA1.RPT_SIGESTA_EJEC008_PLAN_COSECHA, TIPO, ID_ZAFRA, USUARIO_CREA, FECHA_CREA, ZONA, _
        ...
        If lEjecucionAlza.ContainsKey("CARGADORA") Then
            ...
            If lTC > 0 Then
                xrCargaTC.Text = (lTC).ToString(ftoDecimal)
                xrCargaPorcTC.Text = (lTC / lTC_TOTAL).ToString(ftoPorc)
                xrCargaREND_85.Text = (lLBS_85 / lTC).ToString(ftoDecimal)
                xrCargaREND_REAL.Text = (lLBS_REAL / lTC).ToString(ftoDecimal)
                'xrCargaEFICIENCIA.Text = ((lLBS_85 / lTC) / (lLBS_REAL / lTC)).ToString(ftoPorc)
                xrCargaLBS.Text = (lLBS_REAL).ToString(ftoDecimal)
                xrCargaPorcLBS.Text = (lLBS_REAL / lLBS_REAL_TOTAL).ToString(ftoPorc)
            End If
        End If
