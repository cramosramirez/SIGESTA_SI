...
Public Class rptSigestaEjec009_Zafra
    ...
    Public Sub CargarDatos(ByVal TIPO As String, _
        ...
        Me.RPT_SIGESTA_EJEC009_PLAN_COSECHATableAdapter1.FillPorCriterios(DS_SIGESTA1.RPT_SIGESTA_EJEC009_PLAN_COSECHA, TIPO, ID_ZAFRA, USUARIO_CREA, FECHA_CREA, ZONA, _
        ...
        Dim lEjecucionTrans As Dictionary(Of String, Dictionary(Of String, Decimal)) = (New cLOTES_COSECHA).ObtenerEJECUCION_TRANSPORTE(ID_ZAFRA)

        If lEjecucionTrans IsNot Nothing AndAlso lEjecucionTrans.Count > 0 Then
        ...
        For Each p1 As Dictionary(Of String, Decimal) In lEjecucionTrans.Values
        ...
        If lEjecucionTrans.ContainsKey("3 EJES") Then
            lTC = lEjecucionTrans("3 EJES")("TC")
            lLBS_85 = lEjecucionTrans("3 EJES")("LBS_85")
            lLBS_REAL = lEjecucionTrans("3 EJES")("LBS_REAL")
            If lTC > 0 Then
                xr3EjesTC.Text = (lTC).ToString(ftoDecimal)
                xr3EjesPorcTC.Text = (lTC / lTC_TOTAL).ToString(ftoPorc)
                xr3EjesREND_85.Text = (lLBS_85 / lTC).ToString(ftoDecimal)
                xr3EjesREND_REAL.Text = (lLBS_REAL / lTC).ToString(ftoDecimal)
                'xr3EjesEFICIENCIA.Text = ((lLBS_85 / lTC) / (lLBS_REAL / lTC)).ToString(ftoPorc)
                xr3EjesLBS.Text = (lLBS_REAL).ToString(ftoDecimal)
                xr3EjesPorcLBS.Text = (lLBS_REAL / lLBS_REAL_TOTAL).ToString(ftoPorc)
            End If
        End If
        If lEjecucionTrans.ContainsKey("CAMION") Then
            lTC = lEjecucionTrans("CAMION")("TC")
            lLBS_85 = lEjecucionTrans("CAMION")("LBS_85")
            lLBS_REAL = lEjecucionTrans("CAMION")("LBS_REAL")
            If lTC > 0 Then
                xrCamionTC.Text = (lTC).ToString(ftoDecimal)
                xrCamionPorcTC.Text = (lTC / lTC_TOTAL).ToString(ftoPorc)
                xrCamionREND_85.Text = (lLBS_85 / lTC).ToString(ftoDecimal)
                xrCamionREND_REAL.Text = (lLBS_REAL / lTC).ToString(ftoDecimal)
                'xrCamionEFICIENCIA.Text = ((lLBS_85 / lTC) / (lLBS_REAL / lTC)).ToString(ftoPorc)
                xrCamionLBS.Text = (lLBS_REAL).ToString(ftoDecimal)
                xrCamionPorcLBS.Text = (lLBS_REAL / lLBS_REAL_TOTAL).ToString(ftoPorc)
            End If
        End If
        If lEjecucionTrans.ContainsKey("RASTRA") Then
            lTC = lEjecucionTrans("RASTRA")("TC")
            lLBS_85 = lEjecucionTrans("RASTRA")("LBS_85")
            lLBS_REAL = lEjecucionTrans("RASTRA")("LBS_REAL")
            If lTC > 0 Then
                xrRastraTC.Text = (lTC).ToString(ftoDecimal)
                xrRastraPorcTC.Text = (lTC / lTC_TOTAL).ToString(ftoPorc)
                xrRastraREND_85.Text = (lLBS_85 / lTC).ToString(ftoDecimal)
                xrRastraREND_REAL.Text = (lLBS_REAL / lTC).ToString(ftoDecimal)
                'xrRastraEFICIENCIA.Text = ((lLBS_85 / lTC) / (lLBS_REAL / lTC)).ToString(ftoPorc)
                xrRastraLBS.Text = (lLBS_REAL).ToString(ftoDecimal)
                xrRastraPorcLBS.Text = (lLBS_REAL / lLBS_REAL_TOTAL).ToString(ftoPorc)
            End If
        End If
    End Sub
End Class